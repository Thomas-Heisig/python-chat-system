...
class SimpleLoraTrainer(TrainingBackend):
    name = "lora"
    ...
    async def train(
        ...
        for step in range(1, total_steps + 1):
            ...
            epoch = round((step / total_steps) * float(job.hyperparameters.get("epochs", 2.0)), 3)
            await progress_callback(
                {
                    ...
                    "log_line": f"training step {step}/{total_steps}",
                }
            )
